...
var app = express(); //server http par express

var http = require('http').Server(app); //socket.io


var io = require('socket.io')(http); //path


var path = require('path'); //execution commande shell


var shell = require('shelljs'); //variable session par express
...
var sharedsession = require("express-socket.io-session"); //BodyParser


var bodyParser = require("body-parser"); //utilisation du midlware css


app.use(express["static"](path.join(__dirname, 'public', 'css'))); //-------------------appelle du midlware session dans express ::::express session

app.use(session); //-------------------midlware bodyparser
...
var indexIo = io.of('/index'); //----------------midlware session dans /control

controlIo.use(sharedsession(session, {
  autoSave: true
}));
app.get("/control", function (req, res) {
  var sess = req.session; //------------------------reception de l'utilisateur--------------------------------//
  ...
  if (typeof req.body.pseudo != 'undefined' && typeof req.body.mac != 'undefined' && typeof req.body.ip != 'undefined' && req.body.token == '03246') {
    ...
    data = [sess.pseudo, sess.mac, sess.ip, sess.heure]; //emission dans la page admin

    indexIo.emit('connected', data); //controlIo.emit('session', sess);
...
controlIo.on('connection', function (socket) {
  ...
  var time;
  if (heure != "") var time = heure; //------------------------Chrono---------------------------//
  ...
  function decrementer() {
    if (time >= 0) {
      ...
      time--; //emission dans l'utilisateur

      socket.emit('chrono', time); //emission dans l'admin
  ...
  setTimeout(decrementer, 1000); //---------------------deconnection par l'interface admin
...
app.get("/index", function (req, res) {
  ...
  if (req.body.token == '03246') res.sendFile(__dirname + '/index.html');else {
    res.redirect(301, 'http://google.com');
  }
...
var deleteUserMac = function deleteUserMac(mac) {
  controlIo.emit('delete', mac);
};
...
indexIo.on('connection', function (socket) {
  ...
  data = getUser(socket); ///supression en cas d'utilisateur indésirable
...
function insertUser() {
  ...
  db.query('INSERT INTO user SET pseudo=?, email=?, mdp=?', data, function (err, user, field) {});
}
...
function getUser(socket) {
  ...
  db.connect(function (err) {
    if (err) throw err;
    db.query("SELECT * FROM connected WHERE isConnected=1", function (err, result, field) {
      if (err) throw err; //console.log('>> results: ', result );

      var string = JSON.stringify(result); //console.log('>> string: ', string );

      var json = JSON.parse(string); //console.log('>> json: ', json)
      //console.log(result);
